...
export default function Index() {
  ...
  useEffect(() => {
    if (circle) {
      ...
      axios
        .get(
          `https://geocode.maps.co/reverse?lat=${circle.position.latitude}&lon=${circle.position.longitude}&api_key=${process.env.EXPO_PUBLIC_GEOCODE_API_KEY}`,
        )
        .then((res) => {
  ...
  return (
    <View>
      ...
      {placeName ? (
        <ThemedText>
          Kiválasztott környék: {placeName} és {circle?.radiusDisplay}-es
          körzete.
        </ThemedText>
      ) : (
        <ThemedText>Nincs kiválasztva hely.</ThemedText>
      )}
